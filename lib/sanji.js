...
Sanji.prototype.sse = function(req, res) {
  ...
  self.heartbeat(25000);
};

Sanji.prototype.heartbeat = function(interval) {
  var self = this;
  // set heartbeat
  setTimeout(function() {
...
Sanji.prototype.setupDownloadRoute = function(router, route) {
  router.get(route.uri, function(req, res) {
    route.downloadFile.name = route.downloadFile.name || 'no_name';
    res.download(route.downloadFile.path, route.downloadFile.name, function(err) {
      if (err) {
