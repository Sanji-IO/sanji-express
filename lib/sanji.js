...
Sanji.prototype.setupUploadFileRoute = function(router, route) {
  ...
  if (!fileConfig.allowedFiles) {
    config.rename = function(fieldname, filename) {
...
Sanji.prototype.setupDownloadRoute = function(router, route) {
  ...
  router.get(route.uri, function(req, res) {
    ...
    route.downloadFile.filename = req.params.filename || route.downloadFile.filename || 'no_name';

    // for security issue, we strictly limit downloadFile only in bundlePath
    var filePath = path.normalize(path.join(
      route._bundlePath,
      route.downloadFile.path,
      route.downloadFile.filename
    ));

    if (!self.vaildBundlePath(route._bundlePath, filePath)) {
      var errObj = {
        code: 403,
    ...
    res.download(filePath, route.downloadFile.filename, function(err) {
      if (err) {
...
Sanji.prototype.setupDeleteRoute = function(router, route) {
  ...
  router.delete(route.uri, function(req, res) {
    ...
    route.deleteFile.filename = req.params.filename;
...
Sanji.prototype.vaildBundlePath = function(bundlePath, filename) {
  return filename.indexOf(bundlePath) !== -1;
};
