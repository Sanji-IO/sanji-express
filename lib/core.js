...
var SanjiExpressPuppetMaster = require('./plugins/puppetmaster');
var SanjiExpressFile = require('./plugins/file');
var SanjiExpress;
...
module.exports = function(options) {
  ...
  router.use(/^(\/(cg\-[\w]+))?(\/.*)$/, initializeRequest);

  // creates group/remote bundle path
  router.use(SanjiExpressPuppetMaster(bundle, options.io));

  router.use(SanjiExpressDownloadHelper(options.downloadHelperOptions));

  // perform mqtt translate [GET, POST, PUT, DELETE]
  router.use(SanjiExpressFile({
    resource: '*',
    file: {
      _bundlePath: '/tmp/uploads',
      upload: {
        publicLink: true,
        path: './'
      }
