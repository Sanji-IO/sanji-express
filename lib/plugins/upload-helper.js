...
var sanjiHelper = function (req, res, next) {
  ...
  var reqData;
  // Handle multiple files, respond with "files"
  if (Array.isArray(req.filebin)) {
    var files = {};
    req.filebin.forEach(function (file) {
      files[file.fieldname] = {
        url: process.env.SITE_URL + file.url,
        physicalPath: file.physicalPath,
        headers: req.headers
      }
    });

    reqData = objectAssign({
      files: files
    }, jsonData);

  } else {
  // Handle single file, respond with "file"
    reqData = objectAssign(
      {
        file: {
