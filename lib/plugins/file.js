...
var debug = require('debug')('sanji:express:file');
var linkDb = {};
var SanjiExpressFile;

var getRouteFilePath = function(bundlePath, fileConfig, errFn) {
  // upload may not have filename
  var filename = fileConfig.filename || '';
...
var setupDownloadRoute = function(router, route) {
  ...
  debug('route add [%s]: %s', 'download file', route.resource);
};
...
var setupDeleteRoute = function(router, route) {
  ...
  debug('route add [%s]: %s', 'Download', route.resource);
};

var setupUploadRoute = function(router, route) {
  var filenames = [];
  var fileConfig = route.file.upload;
  ...
  var uploadHandler = function(req, res, next) {
    ...
    try {
      if (req.body.formData) {
        ...
        // remove formData
        delete req.mqttData.data.formData;
      }
    } catch (e) {
      res.status(400).json({
  ...
  var filePath = getRouteFilePath(
    ...
    fileConfig,
    function(errObj) {
      debug('upload file path error @%s', route.resource);
      debug(errObj.message);
      debug(errObj.log);
    }
  ...
  var multerConfig = {
    ...
    onFileSizeLimit: function(file) {
      fs.unlink(file.path);
      debug('file size exceeded! %s has been deleted.', file.path);
    },
  ...
  debug('route add [upload file]: %s', route.resource);
};

var createPublicLink = function(router, downloadBase) {
  var downloadRouter = express.Router();
  var downloadBase = downloadBase || '/download';
  ...
  downloadRouter.get('/:filename', function(req, res) {
    ...
    res.download(filepath, linkDb[filename].name, function(err) {
      if (err) {
        var errObj = {
          code: err.status,
          message: 'Download file failed.',
  ...
  return function(router, route) {
    var create = function(req, res, next) {
      ...
      if (req.mqttData.data && !req.mqttData.data._file) {
        return next();
      }

      var files = req.mqttData.data._file.files;
      var fileConfig = route.file.upload;
      var publicLink = req.mqttData.data._file.publicLink = {};
...
var populateIndexData = function(route, req, res, next) {
  ...
  if (!fileConfig) {
    debug('You may have wrong file config in bundle.json');
    return next();
  }
  ...
  filePath = getRouteFilePath(
    ...
    fileConfig,
    function(errObj) {
      debug('populateIndexData path error @%s', route.resource);
      debug(errObj.message);
      debug(errObj.log);
    }
