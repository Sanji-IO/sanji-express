var log = require('sanji-logger')('sanji-rest').child({module: 'loader'}),
    path = require('path'),
    fs = require('fs'),
    _ = require('lodash'),
    Loader;
...
Loader.prototype.startWatch = function startWatch(cb) {
  ...
  .on('all', function(event, path) {
    ...
    if (path.indexOf(
      self.bundleConfigName, path.length - bundleConfigNameLen) === -1) {
      return;
    }
...
Loader.prototype.getRoutes = function getRoutes() {
  ...
  this.bundleConfigs.forEach(function(bundle) {
    ...
    _.map(bundle.resources, function(resource) {
      ...
      // normalize methods
      resource.methods = resource.methods || defaultMethods;
